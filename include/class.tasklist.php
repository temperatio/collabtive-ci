...
class tasklist
{
    ...
    */
    ...
    function getProjectTasklists($project, $status = 1)
    {
        ...
        $sel = mysql_query("SELECT * FROM tasklist WHERE project = $project AND status=$status ORDER BY name ASC");
        /* DAMIAN - END */
        $tasklists = array();
        ...
        while ($list = mysql_fetch_array($sel))
        {
            ...
            $sel2 = mysql_query("SELECT ID FROM tasks WHERE liste = $list[ID] AND status=1 ORDER BY `end` ASC, title ASC");
            /* DAMIAN - END */
            $list['tasks'] = array();
