...
<a href="manageproject.php?action=showproject&amp;id={$project.ID}" title="{$projectname}"><img src="./templates/collabtive-ci/images/symbols/projects.png" alt="" />{$projectname|truncate:25:"...":true}</a>
<a href="managetask.php?action=showproject&amp;id={$project.ID}"><img src="./templates/collabtive-ci/images/symbols/tasklist.png" alt="" />{#tasklists#}</a>
<a href="managetasklist.php?action=showtasklist&id={$project.ID}&tlid={$task.liste}" title="{#tasklist#} / {$task.list}"><img src="./templates/collabtive-ci/images/symbols/tasklist.png" alt="" />{$task.list|truncate:25:"...":true}</a>
<a href="managetask.php?action=showtask&amp;tid={$task.ID}&amp;id={$project.ID}" title="{#task#} / {$task.title}"><img src="./templates/collabtive-ci/images/symbols/tasklist.png" alt="" />{$task.title|truncate:50:"...":true}</a>
<span>&nbsp;/...</span>
</div>

<h1 class="second"><img src="./templates/collabtive-ci/images/symbols/task.png" alt="" />{$task.title|truncate:30:"...":true}</h1>
					...
					<div class="row">
				        ...
				        <select name = "priority" id="priority" required="1" exclude="-1" realname="{#priority#}">
				            ...
				            {section name=priorities loop=$priorities}
				                <option value="{$priorities[priorities].id}" {if $task.priority == $priorities[priorities].value}selected = "selected"{/if}>{$priorities[priorities].value}</option>
				            {/section}
				        </select>
				    </div>
