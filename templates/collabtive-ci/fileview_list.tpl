					...
					{/if}
						<tr >
							<td><img src = "templates/collabtive-ci/images/symbols/folder-sub.png" /></td>
							<td>
					...
					{/if}
						<tr >
							<td><a href = "{$files[file].datei}" {if $files[file].imgfile == 1} rel="lytebox[]" {elseif $files[file].imgfile == 2} rel = "lyteframe[text]" rev="width: 650px; height: 500px;"{/if}>
													 		...
													 		{else}
													 		<img src = "templates/collabtive-ci/images/files/{$files[file].type}.png" alt="{$files[file].name}" />
													 		{/if}
													 	</a></td>
							<td>
