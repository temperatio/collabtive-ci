...
/* DAMIAN - START */
$_project = (object) new project();
$myprojects = $_project->getMyProjects($userid);
...
if ($action == "addform") {
        ...
        if ($settings["mailnotify"]) {
            foreach($assigned as $member) {
                ...
                if (!empty($user["email"]))
                {
                    ...
                    $themail_body = $langfile["hello"] . " " . $user["name"] . ".<br /><br/>";
                    $themail_body .= $langfile["taskassignedtext"] . " <a href = \"" . $url . "managetask.php?action=showtask&id=$id&tid=$tid\">$title</a>";
                    $themail_body .= "<br /><br />" . stripslashes($text);
                    $themail_body .= "<br /><br />" . $langfile["due"] . ": " . date("d.m.y", strtotime($end));
                    $themail_body .= "<br /><br />" . $langfile["priority"] . ": " . $task->getPriority($priority);
                    $themail->send_mail($user["email"], $settings["name"] . " - " . $langfile["taskassignedsubject"] . " - " . $project_data["name"], $themail_body);
                }
                /* DAMIAN - END */
            }
        }
    ...
    /* DAMIAN - START */
    $template->assign("priorities", $task->getPriorities());
    /* DAMIAN - END */
                ...
                if ($assignChk) {
                    if ($settings["mailnotify"]) {
                        ...
                        if (!empty($user["email"]))
                        {
                            ...
                            $themail_body .= "<br /><br />" . $langfile["priority"] . ": " . $task->getPriority($priority);
                            $themail->send_mail($user["email"], $settings["name"] . " - " . $langfile["taskassignedsubject"] . " - " . $project_data["name"], $themail_body);
                            //$themail->send_mail($user["email"], $settings["name"] . " - " . $project_data["name"], $themail_body);
                        }
                        /* DAMIAN - END */
                    }
                }
